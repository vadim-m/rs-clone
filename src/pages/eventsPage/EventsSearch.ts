export class EventsSearch {
  ...
  private createElement(): string {
    ...
    <input class="events__input placeholder:italic placeholder:text-slate-400 block bg-white w-full border border-myslate rounded-md py-2 px-4 shadow-sm focus:outline-none focus:border-myblue focus:ring-myblue focus:ring-1 text-xs dark:bg-slate-500 dark:placeholder:text-slate-300 dark:border-slate-700" placeholder="Найти событие" type="text" name="search"/>
  </label>
    `;
  }
}
