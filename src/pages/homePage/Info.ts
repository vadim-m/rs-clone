  ...
  createElement() {
    ...
    fragment.innerHTML = `
      ...
      <div class="info__car car bg-myslate rounded-lg shadow-md mb-2 p-4 grid grid-areas-layout gap-x-2">
        <h3 class="car__name font-bold text-sm text-right grid-in-name">${this.car?.brand} ${this.car?.model}</h3>
        <p class="car__year text-xs text-right grid-in-year">${this.car?.year}</p>
        ...
        <div class="car__mileage grid-in-mileage">
          ...
          <div class="car__mileage-average leading-3">
            ...
            <span class="text-xs">ХЗ! Пробег за <span>февраль</span></span>
          </div>
        </div>
